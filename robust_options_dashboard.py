#!/usr/bin/env python3
"""
🚀 ROBUST STOCK OPTIONS DASHBOARD
Handles numpy/scipy compatibility issues with fallback calculations
...
def install_packages():
    packages = ['flask', 'yfinance', 'pandas', 'numpy', 'flask-cors', 'pyngrok']
...
print("🚀 Setting up Robust Stock Options Dashboard...")
install_packages()
...
# Fallback normal distribution functions (no scipy dependency)
def norm_cdf(x):
...
class RobustOptionsAnalyzer:
    def __init__(self):
        ...
        # Try to import scipy, use fallback if failed
        self.use_scipy = False
        try:
            from scipy.stats import norm
            self.norm = norm
            self.use_scipy = True
            print("✅ Using scipy for Greeks calculations")
        except ImportError:
    ...
    def calculate_greeks(self, S, K, T, r, sigma, option_type='call'):
        """Calculate option Greeks with fallback implementation"""
        if T <= 0 or sigma <= 0:
        ...
        try:
            ...
            # Use scipy if available, otherwise use fallback
            if self.use_scipy:
                if option_type == 'call':
                    delta = self.norm.cdf(d1)
                    theta = (-(S * self.norm.pdf(d1) * sigma) / (2 * math.sqrt(T)) 
                            - r * K * math.exp(-r * T) * self.norm.cdf(d2)) / 365
                else:  # put
                    delta = self.norm.cdf(d1) - 1
                    theta = (-(S * self.norm.pdf(d1) * sigma) / (2 * math.sqrt(T)) 
                            + r * K * math.exp(-r * T) * self.norm.cdf(-d2)) / 365
                
                gamma = self.norm.pdf(d1) / (S * sigma * math.sqrt(T))
                vega = S * self.norm.pdf(d1) * math.sqrt(T) / 100
            else:
                ...
                try:
                    ...
                    for _, call in option_chain.calls.iterrows():
                        ...
                        call_data = {
                            ...
                            'lastPrice': float(call.get('lastPrice', 0)),
                            'bid': float(call.get('bid', 0)),
                            'ask': float(call.get('ask', 0)),
                            'volume': int(call.get('volume', 0)) if not pd.isna(call.get('volume', 0)) else 0,
                            'openInterest': int(call.get('openInterest', 0)) if not pd.isna(call.get('openInterest', 0)) else 0,
                    ...
                    for _, put in option_chain.puts.iterrows():
                        ...
                        put_data = {
                            ...
                            'lastPrice': float(put.get('lastPrice', 0)),
                            'bid': float(put.get('bid', 0)),
                            'ask': float(put.get('ask', 0)),
                            'volume': int(put.get('volume', 0)) if not pd.isna(put.get('volume', 0)) else 0,
                            'openInterest': int(put.get('openInterest', 0)) if not pd.isna(put.get('openInterest', 0)) else 0,
                            'impliedVolatility': float(iv),
                            'delta': greeks['delta'],
                            'gamma': greeks['gamma'],
                            'theta': greeks['theta'],
...
app = Flask(__name__)
CORS(app)
analyzer = RobustOptionsAnalyzer()
...
<head>
    ...
    <title>📈 Robust Options Dashboard</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>📈 Robust Options Dashboard</h1>
            <p>Compatibility-Fixed Options Analysis with Greeks</p>
        </div>
        
        <div class="status-bar">
            ✅ System Ready - Numpy/Scipy compatibility issues resolved
        </div>
        ...
        <div class="notes-section">
            ...
            <div style="background: #d4edda; padding: 15px; border-radius: 8px; border: 1px solid #c3e6cb;">
                <h4 style="color: #155724; margin-bottom: 10px;">🛠️ Technical Notes</h4>
                <p style="color: #155724;">This version uses robust fallback calculations for Greeks when scipy is unavailable, ensuring compatibility across all Python environments.</p>
            </div>
        </div>
    </div>
...
def main():
    print("\n🚀 Starting Robust Stock Options Dashboard...")
    ...
    print("\n" + "="*50)
    print("✅ ROBUST DASHBOARD IS LIVE!")
    print("="*50)
    ...
    print("• Numpy/Scipy compatibility resolved")
    print("• Fallback Greeks calculations implemented")
